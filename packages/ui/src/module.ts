...
export default defineNuxtModule<ModuleOptions>({
  ...
  async setup(options, nuxt) {
    ...
    nuxt.options.alias["#ui/elements"] = resolve(runtimeDir, "components/elements/ui");

    nuxt.options.alias["#ui/composables"] = resolve(runtimeDir, "composables");
    nuxt.options.alias["#ui/locales"] = resolve(runtimeDir, "locales");
    ...
    // 3. COMPONENTS
    addComponentsDir({
      path: resolve(runtimeDir, "components/elements/ui"),
