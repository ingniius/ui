...
export async function validateStandardSchema(
  ...
  if (result.issues) {
    return {
      errors: result.issues?.map(issue => ({
        name: issue.path?.map(item => isObject(item) ? item.key : item).join(".") || "",
  ...
  return { errors: null, result: result.value };
}
...
async function validateYupSchema(
  ...
  try {
    ...
    if (isYupError(error)) {
      const errors = error.inner.map(issue => ({ name: issue.path ?? "", message: issue.message }));
...
async function validateSuperstructSchema(state: any, schema: Struct<any, any>): Promise<ValidateReturnSchema<typeof state>> {
  ...
  if (err) {
    const errors = err.failures().map(error => ({ message: error.message, name: error.path.join(".") }));
    return { errors, result: null };
  }
...
async function validateJoiSchema(
  ...
  try {
    ...
    if (isJoiError(error)) {
      const errors = error.details.map(issue => ({ name: issue.path.join("."), message: issue.message }));
      return { errors, result: null };
    } else {
      throw error;
    }
