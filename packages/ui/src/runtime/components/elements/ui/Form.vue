...
import type { Form, FormError, FormErrorEvent, FormErrorWithId, FormEvent, FormInputEvents, FormSchema, FormSubmitEvent } from "../../../types/form";

import { tv } from "../../../utils/class";

const appConfig = _appConfig as AppConfig & {
  ui: { components: { form: Partial<typeof theme> } };
};

const form = tv({
  extend: tv(theme),
  ...(appConfig.ui?.components?.form || {}),
});
...
const props = withDefaults(defineProps<FormProps<T>>(), {
  validateOn: () => ["input", "blur", "change"] as FormInputEvents[],
