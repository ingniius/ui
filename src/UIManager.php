...
final class UIManager
{
    ...
    public function tv(array $config = []): Closure
    {
        ...
        if ($isFullConfig) {
            if (isset($finalConfig['base'])) {
                $finalConfig['slots']['root'] = $this->cn($finalConfig['slots']['root'] ?? '', $finalConfig['base']);
            ...
            return $this->tvCache[$key] = fn (array $recipes = []) => new CvaSlot(array_map(
                fn (string|array $slotClass) => fn (string|array $override = ''): string => $this->cn($slotClass, $override),
        ...
        return $this->tvCache[$key] = fn (array $recipes = []) => new CvaSlot([
            'root' => fn (string|array $override = ''): string => $this->cn($cvaInstance->apply($recipes), $override),
        ]);
    }
     ...
     */
    public function cn(string|array ...$args): string
    {
    ...
    public function recursiveMerge(array $array1, array $array2): array
    {
        ...
        foreach ($array2 as $key => $value) {
            if (is_array($value) && isset($merged[$key]) && is_array($merged[$key])) {
                ...
                $merged[$key] = $this->cn([$merged[$key], $value]);
            } else {
                $merged[$key] = $value;
            }
        }
     ...
     * @param  (?string)  ...$path
     */
    public function refinePath(?string $path): ?string
    {
