...
final class ScriptAsset
{
    ...
    {
        $manifestContent = file_get_contents(__DIR__.'/../../dist/manifest.json') ?: '{}';
        ...
        if (config('app.debug')) {
            return "<script src=\"/vee/ui.js?id={$versionHash}\" data-navigate-once{$nonce} defer></script>";
        }

        return "<script src=\"/vee/ui.min.js?id={$versionHash}\" data-navigate-once{$nonce} defer></script>";
