...
final class CvaExtend
{
    ...
    public function apply(array $recipes = []): array
    {
        ...
        foreach ($rawSlots as $slotName => $baseClasses) {
            $processedDefinitions[$slotName] = $this->emptySlot($rawDefaultVariants, $baseClasses);
        }
        ...
        foreach ($processedDefinitions as $slotName => $definition) {
            ...
            $applied = $cvaInstance->apply($this->normalizeVariants($recipes));
            $resolvedClasses[$slotName] = is_array($applied) ? ($applied[$slotName] ?? '') : $applied;
        }
