...
@php
    $self = Vee\UI::tv(['extend' => Vee\Theme\Avatar::theme(), ...useComponents('avatar')])(['size' => $size]);
    ['root' => $rootClass, 'image' => $imageClass, 'icon' => $iconClass, 'fallback' => $fallbackClass] = array_merge(
        ['root' => '', 'image' => '', 'icon' => '', 'fallback' => ''],
        $ui,
    );
...
<vee:button-or-link :$as :$href :attributes="$attributes->class($self->root($rootClass))" data-ui-avatar>
    @if ($src)
